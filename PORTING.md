...
**可编辑项**：站点位置（拖拽 / 方向键，`Shift` 加速 10 倍）、中英文站名、车站类型（`dot`/`tsf`/`rdot`/`no`）、站名朝向（8 方向轮盘）、文字偏移 `offset`（`Alt` + 方向键）、删除车站（`Delete`，同步从线路 `stationIds` 与 `distances` 中摘除）。`Ctrl/Cmd + Z` 撤销，`Ctrl/Cmd + S` 导出。
...
> 2. **挪动车站不会自动改动 `pathPoints` 折线**。线路折线是独立几何，导出预览会明确列出受影响的线路，需要时请在 `data_lines.js` 中同步折点。
