...
> [!IMPORTANT]
> **🚨 移植与二次开发重要大前提（最高铁律）**：  
> **本项目所有内容必须为纯前端，禁止使用任何其他技术（如 Node.js 服务端、PM2 进程守护/管理、Express 后端服务、React/Vue/Angular 等重型框架及打包编译器）**！  
> **唯一例外**：若涉及**监控车站运营状态、监控网页**等后台任务，**可以破例**使用 Node.js / PM2 进行辅助脚本编写与守护；**其他所有主要地图功能（核心渲染、线网走向、车站信息板、数据层等）一律严格禁止破例**，必须全部为原生 JavaScript (ES6+)、CSS 与 SVG 静态实现。
...
Drunk 不只用来做新城市——它同时是 **OpenMap 任意已注册城市的可视化编辑器**，用于上线之后的日常维护（挪站、改字、调站名朝向）。

**入口**（任选其一）：
- 线路图右上角「更多 → 偏好设置 → 编辑此图 → 打开编辑模式」；
- Drunk 顶栏城市下拉框选中后点「编辑此图」；
- 直接访问 `drunk/index.html?city={city_id}`。

**车站可编辑项**：位置（拖拽 / 方向键，`Shift` 加速 10 倍）、中英文站名、车站类型（`dot`/`tsf`/`rdot`/`no`）、站名朝向（8 方向轮盘）、文字偏移 `offset`（`Alt` + 方向键）、删除车站（`Delete`，同步从线路 `stationIds` 与 `distances` 中摘除）。

**线路可编辑项**：点右侧图例项或直接点画布上的线条选中，可改线路名称、标志色（取色器 / 手填，手填保留 `rgb(...)` 等原有写法）、运营公司；选中后线条上出现走向折点手柄——拖动折点调整走向、方向键微调、**双击线条在该处插入折点**、`Delete` 删除折点。分支线路的 `pathPoints-main` / `-branch1` / `-branch2` 全部支持。

**仅落站点**：勾选后该线路只在图上落站点图元、不画走向（对应 `isPointOnly`），国铁这类散布全城的车站集合用它——否则按站序直连会拉出横穿全图的长斜线。

**拐角圆角**：拐角折点显示为圆形手柄（已手动指定过 `r` 的显示为琥珀色），选中后可用滑块 / 数值框 / 快捷预设调该拐角的圆角半径。留空=自动（90° 取 18px、斜角取 8px），`0`=保持直角。相邻线段过短时引擎会自动收窄半径避免圆角重叠，面板会明确提示「当前生效 xx px」。整条线还可勾选「收紧倒角」（对应 `useStrictRounding`）。

`Ctrl/Cmd + Z` 撤销，`Ctrl/Cmd + S` 导出，`Esc` 取消选中。

**滚轮与触控板**：复用主图「偏好设置 → 滚轮与触控板」的同一项设置（`smart` / `zoom` / `pan`）。smart 下触控板两指滑动走平移、鼠标滚轮走缩放，触控板捏合恒为缩放。
...
> 2. **挪动车站不会自动改动 `pathPoints` 折线**。线路折线是独立几何，导出预览会明确列出受影响的线路；选中该线路后直接拖折点即可同步走向。
...
const CITY_REGISTRY = {
    "shanghai": {
        ...
        svglogo: null, // 城市官方矢量徽标：尽量不使用 SVG，优先使用 CGoUI 内置图标（设为 null 即可；若 CGoUI 未收录方填自定义 SVG，留空默认显示小火车图标）
...
2. **`svglogo` (城市官方矢量徽标 / 图标配置)**：
   - **核心规范（参与者重要要求）**：**城市图标尽量不使用自定义 SVG，优先使用 CGoUI 的官方内置图标**；如果 CGoUI 库中没有该城市图标，方可使用自定义 SVG。
   - **优先使用 CGoUI 内置图标**：
     - CGoUI 现已原生内置收录 42+ 座城市轨道交通官方单色矢量标志（涵盖北京、上海、广州、深圳、天津、重庆、成都、武汉、南京、杭州、西安、郑州、长沙、沈阳、青岛、大连、合肥、福州、厦门、苏州、无锡、宁波、常州、南通、温州、芜湖、济南、太原、石家庄、哈尔滨、长春、南昌、南宁、贵阳、昆明、兰州、东莞、佛山、香港、台北、高雄、桃园等）；
     - 若该城市已由 CGoUI 收录，**请务必将 `svglogo` 配置为 `null` 或留空**，系统将自动调用 `<cgo-icon name="{city_id}" size="22">`，具备极佳的加载性能与深浅色模式自适应能力。
   - **自定义 SVG 的降级使用**：
     - **仅当** CGoUI 官方图标库中确实未收录该城市轨道交通标志（如海外城市悉尼、墨尔本等，或新兴城轨）时，才允许在 `svglogo` 字段中填写自定义 SVG 字符串；
     - 自定义 SVG 须遵循：① 去除写死的 `fill` 颜色，引擎自动注入 `currentColor`；② 去除 `viewBox` 与 XML 头部声明（引擎在渲染时自动自适应充满 `22px × 22px` 视口）；
     - 若未填入且 CGoUI 也未收录，系统将安全降级显示经典小火车图标。
...
1. **🌟 尊享官方「城市主理人」专属署名**：
   ...
   - *（注：获得主理人署名的同时需承担长期更新维护职责；若出现数据质量堪忧或长期弃管，将依规触发下架整改或主理人移交流转，详见 [CONTRIBUTING.md](./CONTRIBUTING.md)）*
