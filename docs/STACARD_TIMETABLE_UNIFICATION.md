...
| 9 | 是否升级 `core/cgo-ui.js` 以取得上游新图标 | 待定。核实结果：本地那份（124 图标）与 npm 已发布最新版 `@centralgo/cgo-ui@2.1.1` **图标集逐名相同**；而 CGoUI 文档站跑的是未发布的 dev 构建，有 **157** 个图标，多出 33 个且与本项目高度相关——`tram`、`railway`、`bus`、`walk`、`stairs`、`escalator`、`elevator`、`toilet`、`baby`、`stroller`、`luggage`、`tourist`、`security`、`police`、`pregnant`、`elder`、`aed`、`counter`、`monorail`、`crh`、`noentry`、`plane`、`light`、`world`、`drunk`，以及 `beijing`/`changchun`/`dalian`/`fuzhou`/`hefei`/`qingdao`/`shanghai`/`shenyang` 城市图标。这几乎正好覆盖 AGENTS.md 6.1 车站信息板的 7 大领域（站台结构、设施指南、接驳空间），建议等上游发布后整体升级，**不要单独手工补键**（会与 vendored 版本产生偏离） |
