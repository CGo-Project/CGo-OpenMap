...
(function () {
     ...
     * 主理人虚位以待时退化为贡献指南入口。
     *
     * @param {string} what - 缺失内容的一句话说明
     */
    function contributionHintHtml(what) {
        ...
        const linkStyle = "color: var(--primary-color); text-decoration: none;";
        const contact = owner
            ? (owner.github
                ? `城市主理人 <a href="${owner.github}" target="_blank" style="${linkStyle}">${escapeHtml(owner.name)}</a>`
                : `城市主理人 ${escapeHtml(owner.name)}`)
            : `<a href="./CONTRIBUTING.md" target="_blank" style="${linkStyle}">项目贡献指南</a>`;
        return `<div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed currentColor; opacity: 0.8; font-size: 11px; line-height: 1.6;">`
            + `${escapeHtml(what)}<br>`
            ...
            render({ station }) {
                ...
                if (!person.name) {
                    return tipCard.render({
                        ...
                        icon: "edit",
                        iconSize: 14,
                        body: pendingText + contributionHintHtml("本站题写者的落款、印章或站内说明牌尚待考证。")
                    });
                }
                ...
                const hint = info.image
                    ? ""
                    : contributionHintHtml("本站题字横图尚未收录，标题栏暂以普通文字显示站名。");
