...
(function () {
    ...
    const ShenyangCity = {
        ...
        /**
         * 换乘站图元接管：把通用双圈换成带线路色的换乘徽标。
         * 仅处理 tsf，其余站型返回 null 回落引擎通用模板（core/ 不做城市定制）。
         */
        renderStationIcon(station) {
            if (!station || station.type !== "tsf") return null;
            const colors = Array.isArray(station.lineColors) ? station.lineColors : [];
            const top = colors[0] || "var(--station-stroke)";
            const bottom = colors[1] || colors[0] || "var(--station-stroke)";
            const b = TRANSFER_BADGE;
            return {
                html: `<svg viewBox="${b.viewBox}" xmlns="http://www.w3.org/2000/svg">`
                    + `<path d="${b.bg}" fill="var(--map-bg)"/>`
                    + `<path d="${b.arrowTop}" fill="${top}" fill-rule="evenodd"/>`
