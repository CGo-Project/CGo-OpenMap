 ...
 *   - 其余               → 「XX站」
 *
 * 注：54 路、55 路有轨电车数据尚未录入，录入时请把线路 ID 补进
 *     changchun.js 的 TRAM_LINES，有轨站标题即会自动生效。
 */
(function () {
    "use strict";

    /** 与火车站同名的地铁站标题形式：prefix = 「地铁XX站」 / suffix = 「XX站（轨道交通）」 / none = 不特殊处理 */
    const METRO_RAIL_FORM = "suffix";
    ...
    function isTramStation(station) {
        const city = window.CHANGCHUN_CITY || window.CURRENT_CITY;
    ...
    function install() {
        if (!document.body || document.body.dataset.changchunTitleObserver === "true") return;
        document.body.dataset.changchunTitleObserver = "true";
    ...
    window.ChangchunStationTitle = { buildTitle, normalizeTitles };
